...
from model.scGAN import Generator, Discriminator
...
class TreeGAN:
    def __init__(self, args):
        ...
        if self.args.krepul_loss:
            # PU-net
            self.krepul_loss = kNNRepulsionLoss(
                k=self.args.krepul_k,
                n_seeds=self.args.krepul_n_seeds,
                h=self.args.krepul_h,
            )
    ...
    def run(self, save_ckpt=None, load_ckpt=None):
        ...
        for epoch in range(epoch_log, self.args.epochs):
            ...
            for _iter, data in enumerate(self.dataLoader):
                ...
                if self.args.expansion_penality:
                    dist, _, mean_mst_dis = self.expansion(
                        fake_point,
                        self.args.expan_primitive_size,
                        self.args.expan_alpha,
                    )
            ...
            print(
                ...
                "{:3}".format(epoch),
                "[ D_Loss ] ",
                "{: 7.6f}".format(d_loss_mean),
                "[ G_Loss ] ",
                "{: 7.6f}".format(g_loss_mean),
                "[ Time ] ",
            ...
            if self.args.eval_every_n_epoch > 0:
                if epoch % self.args.eval_every_n_epoch == 0:
                    checkpoint_eval(
                        ...
                        conditional=False,
                        ratio="even",
                        FPD_path=self.args.FPD_path,
                        class_choices=self.args.class_choice,
                    )
...
if __name__ == "__main__":
    ...
    SAVE_CHECKPOINT = (
        args.ckpt_path + args.ckpt_save if args.ckpt_save is not None else None
    )
